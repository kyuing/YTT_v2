  ...
  if(req.query.q === "scripts") {
    ...
    (async () => {
      try {
        ...
        // /ytt/api/:id/scripts
        // const {data} = await axios.get('https://ytt-api.herokuapp.com/ytt/api/' + req.params.id);
        // data.videoId = data.videoId;
        // data.title = data.title;
        // data.url = data.url;

        //this wway works, url (http status) is too ling, though.
  ...
  (async () => {
    try {
      ...
      if(data) {
        ...
        res.redirect("/ytt/" + data._id + "?q=scripts&videoId=" + data.videoId + "&url=" + url + "&title=" + data.title);  //to getDoc
        // res.redirect("/ytt/" + id + "?q=scripts");  //to getDoc
        // res.redirect("/ytt/api/" + data._id + "/scripts");  //to getDoc
